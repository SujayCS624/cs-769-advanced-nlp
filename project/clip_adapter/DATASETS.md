...
### RSNA-P
- Create a folder named `rsna_pneumonia/` under `$DATA`.
- Download the RSNA Pneumonia dataset from https://www.kaggle.com/competitions/rsna-pneumonia-detection-challenge/overview and extract it locally.
- Convert all the DICOM images in train and test to JPEG.
- Structure the dataset within the `rsna_pneumonia/` folder as follows:
```
rsna_pneumonia/
|-- train_jpg/ # folder containing training images
|-- test_jpg/ # folder containing test images
|-- test.csv # csv file with training labels
|-- train.csv # csv file with test labels
```