...
python setup.py \
    --emb "https://dl.fbaipublicfiles.com/fasttext/vectors-english/crawl-300d-2M.vec.zip"

# Step 2. Train models on two datasets.
##  2.1. Run experiments on SST
PREF='sst'
python main.py \
    --emb_file "pre_trained_embeddings/crawl-300d-2M.vec" \
    --emb_size 300 \
...
for file in 'main.py' 'model.py' 'vocab.py' 'setup.py' 'run_exp.sh' 'README.txt' 'NLP_Assignment_1_Report.pdf'; do
	cp $file ${CAMPUSID}/
done
