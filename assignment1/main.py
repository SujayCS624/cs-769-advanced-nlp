...
def pad_sentences(sents, pad_id):
    ...
    # Calculate the number of words in the longest sentence of the mini-batch
    max_sent_size = max(len(sent) for sent in sents)
    aug_sents = []

    # Add padding to make all sentences the same length in the mini-batch
    for sent in sents:
        aug_sent = sent + [pad_id] * (max_sent_size - len(sent))
